...
import { SafeAreaView, Text, FlatList, View, TouchableOpacity, StyleSheet, ActivityIndicator, Alert, Image } from "react-native";
import { fetchMatchesSearchApi, fetchMyProfileApi, sendFriendRequestApi, getAbsolutePhotoUrl } from "../api/api";
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (__DEV__) console.log("new matches load error:", e?.response?.data || e?.message);
      } finally {
        setLoading(false);
      }
    };
